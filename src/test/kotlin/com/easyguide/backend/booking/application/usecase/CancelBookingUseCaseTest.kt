package com.easyguide.backend.booking.application.usecase

import com.easyguide.backend.booking.application.dto.CancelBookingCommand
import com.easyguide.backend.booking.domain.exception.BookingCancelAfterSlotStartException
import com.easyguide.backend.booking.domain.exception.InvalidBookingStatusException
import com.easyguide.backend.booking.domain.model.Booking
import com.easyguide.backend.booking.domain.model.BookingStatus
import com.easyguide.backend.booking.domain.model.CancelledBy
import com.easyguide.backend.booking.domain.repository.InMemoryBookingRepository
import com.easyguide.backend.shared.application.port.FakeClock
import com.easyguide.backend.shared.exceptions.exception.AccessDeniedDomainException
import com.easyguide.backend.tour.domain.model.Tour
import com.easyguide.backend.tour.domain.model.TourCategory
import com.easyguide.backend.tour.domain.model.TourStatus
import com.easyguide.backend.tour.domain.repository.InMemoryTourRepository
import com.easyguide.backend.tourslot.domain.model.TourSlot
import com.easyguide.backend.tourslot.domain.repository.InMemorySlotRepository
import java.math.BigDecimal
import java.time.Instant
import java.time.ZoneId
import java.time.temporal.ChronoUnit
import java.util.UUID
import kotlin.test.Test
import kotlin.test.assertEquals
import kotlin.test.assertFailsWith

class CancelBookingUseCaseTest {

    private val bookingRepository = InMemoryBookingRepository()
    private val slotRepository = InMemorySlotRepository()
    private val tourRepository = InMemoryTourRepository()
    private val clock = FakeClock(Instant.parse("2026-01-01T00:00:00Z"))
    private val useCase = CancelBookingUseCase(bookingRepository, slotRepository, tourRepository, clock)

    private fun tour(guideId: UUID = UUID.randomUUID()): Tour {
        val tour = Tour(
            id = UUID.randomUUID(),
            guideId = guideId,
            title = "Тур",
            description = "Описание тура подлиннее пятидесяти символов для валидации полей",
            city = "Москва",
            category = TourCategory.WALKING,
            meetingPoint = "Точка встречи",
            timezone = ZoneId.of("Europe/Moscow"),
            durationMinutes = 60,
            price = BigDecimal("1000.00"),
            maxPeople = 5,
            status = TourStatus.PUBLISHED,
        )
        tourRepository.save(tour)
        return tour
    }

    private fun slot(tourId: UUID, startsAt: Instant = clock.now().plus(3, ChronoUnit.HOURS)): TourSlot {
        val slot = TourSlot(
            id = UUID.randomUUID(),
            tourId = tourId,
            startsAt = startsAt,
            capacity = 5,
        )
        slotRepository.save(slot)
        return slot
    }

    private fun booking(slotId: UUID, userId: UUID, status: BookingStatus = BookingStatus.PENDING): Booking {
        val booking = Booking(
            id = UUID.randomUUID(),
            slotId = slotId,
            userId = userId,
            seats = 2,
            totalPrice = BigDecimal("2000.00"),
            contactPhone = null,
            comment = null,
            createdAt = Instant.now(),
            status = status,
        )
        bookingRepository.save(booking)
        return booking
    }

    private fun command() = CancelBookingCommand(reason = "Передумал")

    @Test
    fun `турист успешно отменяет свою бронь и места освобождаются`() {
        val tour = tour()
        val slot = slot(tour.id)
        val userId = UUID.randomUUID()
        val booking = booking(slot.id, userId)

        val result = useCase.execute(booking.id, userId, command())

        assertEquals(BookingStatus.CANCELLED, result.status)
        assertEquals(CancelledBy.TOURIST, result.cancelledBy)
        assertEquals(listOf(slot.id to 2), slotRepository.releaseSeatsCalls)
    }

    @Test
    fun `гид тоже может отменить бронь`() {
        val guideId = UUID.randomUUID()
        val tour = tour(guideId)
        val slot = slot(tour.id)
        val booking = booking(slot.id, UUID.randomUUID())

        val result = useCase.execute(booking.id, guideId, command())

        assertEquals(CancelledBy.GUIDE, result.cancelledBy)
    }

    @Test
    fun `чужая бронь (ни турист, ни гид) кидает AccessDeniedDomainException`() {
        val tour = tour()
        val slot = slot(tour.id)
        val booking = booking(slot.id, UUID.randomUUID())

        assertFailsWith<AccessDeniedDomainException> {
            useCase.execute(booking.id, UUID.randomUUID(), command())
        }
    }

    @Test
    fun `неверный исходный статус кидает InvalidBookingStatusException`() {
        val tour = tour()
        val slot = slot(tour.id)
        val userId = UUID.randomUUID()
        val booking = booking(slot.id, userId, status = BookingStatus.COMPLETED)

        assertFailsWith<InvalidBookingStatusException> {
            useCase.execute(booking.id, userId, command())
        }
    }

    @Test
    fun `отмена после начала слота кидает BookingCancelAfterSlotStartException`() {
        val tour = tour()
        val slot = slot(tour.id, startsAt = clock.now().minus(1, ChronoUnit.HOURS))
        val userId = UUID.randomUUID()
        val booking = booking(slot.id, userId)

        assertFailsWith<BookingCancelAfterSlotStartException> {
            useCase.execute(booking.id, userId, command())
        }
    }
}
